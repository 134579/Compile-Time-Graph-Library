...
int main() {
...

  {
    // Example 1
    // -------------------------------------------------------------------------
    // The nodes of a CTGL graph are ctgl::Node<> types with unique ID
    // parameters.
  ...
  }
  {
    ...
    // '----------------------(3)-----------------------'
    using routes =
        ctgl::List<ctgl::Edge<dubai, miami, 1>, ctgl::Edge<miami, paris, 2>,
                   ctgl::Edge<miami, tokyo, 1>, ctgl::Edge<paris, dubai, 3>,
                   ctgl::Edge<paris, tokyo, 1>, ctgl::Edge<tokyo, dubai, 3>,
                   ctgl::Edge<tokyo, paris, 4>>;

    // |circuit| represents an optimal solution to the TSP instance from Dubai.
    // The type of |circuit| will express Dubai --> Miami --> Paris --> Tokyo
    // --> Dubai.
    using world = ctgl::Graph<cities, routes>;
    constexpr auto circuit =
        ctgl::algorithm::findShortestRoute(world{}, dubai{}, cities{});
  ...
  }
  {
    ...
    // Edges represent logarithmic exchange rates between currencies. Why? If
    // the product of the exchange rates along a cycle is less than one, then
    // the sum of the logarithmic exchanges rates along that cycle must be less
    // than zero.
    //                v----------(-2)----------.
    // NZD --(1)--> AUD --(-3)--> CAD --(3)--> USD
    using log_rates =
        ctgl::List<ctgl::Edge<nzd, aud, 1>, ctgl::Edge<aud, cad, -3>,
                   ctgl::Edge<cad, usd, 3>, ctgl::Edge<usd, aud, -2>>;

    // |arbitrage| represents the possibility of arbitrage in the currency
    // market. Here, |arbitrage| will be set to true because AUD --> CAD --> USD
    // --> AUD forms a negative cycle.
    using market = ctgl::Graph<currencies, log_rates>;
    constexpr bool arbitrage = ctgl::graph::hasNegativeCycle(market{});
    std::cout << "The foreign exchange market " << (arbitrage ? "is" : "is not")
              << " susceptible to arbitrage.\n";
  }

...
template <typename... Ts> constexpr int size(List<Ts...>) noexcept;

// Reports whether the given List is empty.
template <typename... Ts> constexpr bool empty(List<Ts...>) noexcept;
...
template <typename... Ts> constexpr auto permutations(List<Ts...>) noexcept;
...
template <typename T> constexpr auto remove(T, List<>) noexcept {
  return List<>{};
}
...
template <typename T> constexpr bool contains(T, List<>) noexcept {
  return false;
}
...
constexpr auto unique(List<>) noexcept { return List<>{}; }

template <typename... Ts> constexpr auto permutations(List<Ts...>) noexcept {
  return permutations(List<>{}, List<Ts...>{});
}

template <typename T> constexpr auto permutations(List<T>) noexcept {
  return List<List<T>>{};
}
...
// Streams the names of the types that compose the given List to the provided
// output stream.
template <typename T, typename... Ts,
          typename = std::enable_if_t<sizeof...(Ts) != 0>>
inline std::ostream &operator<<(std::ostream &out,
                                const List<T, Ts...> &list) noexcept {
...
// -------------------------------------------------------------------------
template <typename... Ts> using List = ctgl::list::List<Ts...>;
} // namespace ctgl
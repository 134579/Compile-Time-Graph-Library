...
template <typename N, typename E> struct Graph {
  using Nodes = N;
  using Edges = E;
};
...
template <typename G> constexpr bool hasCycle(G) noexcept;

// Reports whether the provided Graph has a negative cycle.
template <typename G> constexpr bool hasNegativeCycle(G) noexcept;

// Reports whether the provided Graph is a strongly-connected component.
template <typename G> constexpr bool isConnected(G) noexcept;
...
template <typename N> constexpr auto getAdjacentNodes(N, List<>) noexcept {
...
constexpr auto getConnectedNodes(G, N, List<T, Ts...>, List<Ps...>) noexcept {
  constexpr auto skip =
      getConnectedNodes(G{}, N{}, List<Ts...>{}, List<Ps...>{});
...
template <typename N> constexpr auto getOutgoingEdges(N, List<>) noexcept {
  return List<>{};
}
...
template <typename G> constexpr bool hasNegativeCycle(G, List<>) noexcept {
  // The set of Nodes which could be part of a negative cycle is empty.
  return false;
}

template <typename G, typename N, typename T, typename H, int W, typename... Es,
          typename... Ps>
constexpr bool hasNegativeCycle(G, N, List<Edge<T, H, W>, Es...>,
                                Path<Ps...>) noexcept {
  ...
  if constexpr (cycle) {
    ...
    constexpr bool take =
        hasNegativeCycle(G{}, N{}, edges, Path<Ps..., Edge<T, H, W>>{});
    constexpr bool skip =
        hasNegativeCycle(G{}, N{}, List<Es...>{}, Path<Ps...>{});
    return take || skip;
  }
}

template <typename G, typename N, typename T, int W, typename... Es,
          typename... Ps>
constexpr bool hasNegativeCycle(G, N, List<Edge<T, N, W>, Es...>,
                                Path<Ps...>) noexcept {
...
// -------------------------------------------------------------------------
template <int ID> using Node = ctgl::graph::Node<ID>;

template <typename T, typename H, int W>
using Edge = ctgl::graph::Edge<T, H, W>;

template <typename N, typename E> using Graph = ctgl::graph::Graph<N, E>;
} // namespace ctgl
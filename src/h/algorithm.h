...
namespace algorithm {
template <typename G, typename S, typename T, typename... Ps,
          typename = ctgl::util::enable_if_diff_t<S, T>>
...
template <typename G, typename S, typename T, typename E, typename... Es,
          typename... Ps, typename = ctgl::util::enable_if_diff_t<S, T>>
constexpr auto findShortestPath(G, S, T, List<E, Es...>, List<Ps...>) noexcept {
  // Compute the shortest path that ignores the current Edge.
  constexpr auto skip =
      findShortestPath(G{}, S{}, T{}, List<Es...>{}, List<Ps...>{});
  ...
  if constexpr (cycle) {
    ...
    constexpr auto take =
        findShortestPath(G{}, typename E::Head{}, T{}, next, List<Ps..., E>{});
...
template <typename G> constexpr auto findShortestRoutes(G, List<>) noexcept {
  return path::DNE;
}

...
template <typename... Es> using Path = ctgl::List<Es...>;

// DNE represents an invalid Path (e.g., the path between two disconnected
// nodes).
namespace {
struct invalid {};
} // namespace
...
template <typename... Es> constexpr int length(Path<Es...>) noexcept;

// Finds the unique Nodes in the given Path.
template <typename... Es> constexpr auto nodes(Path<Es...>) noexcept;

// Drops all Edges prior to the given Node in the provided Path.
template <typename T, typename... Es>
constexpr auto dropPrefix(T, Path<Es...>) noexcept;

template <typename T> constexpr auto dropPrefix(T, Path<>) noexcept;
...
// Chooses the "shorter" of the given Paths using the following tabular
// expression:
// +--------------------------------------------------+     +----+
// | p1 == DNE                                        | --> | p2 |
// +-----------+--------------------------------------+     +----+
...
template <> constexpr int length(Path<>) noexcept { return 0; }

template <typename E, typename... Es>
constexpr auto nodes(Path<E, Es...>) noexcept {
  return list::unique(List<typename E::Tail, typename E::Head>{} +
                      nodes(Path<Es...>{}));
}

template <> constexpr auto nodes(Path<>) noexcept { return List<>{}; }
...
// -------------------------------------------------------------------------
template <typename... Es> using Path = ctgl::path::Path<Es...>;
} // namespace ctgl